...
export default function Engagement() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true)
    try {
      const res = await api.get("/get_chat_engagement", { params: { month, group } })
  ...
  useEffect(() => {
    registerEriContext(() => ({
      page: "engagement_rates",
      endpoint: "/api/get_chat_engagement",
      parameters: { month, group },
      description: `Viewing chat engagement data for ${group} in ${month}`,
    }))
    return () => registerEriContext(null)
  }, [group, month])
  const config = {
    avg_messages_per_user: { label: t("Average Messages per User"), color: "rgba(255,99,132,0.8)" },
  } satisfies ChartConfig
  const handleCSV = () =>
    downloadCSV("engagement_rate.csv", ["Channel", "Rate"], rows.map((r) => [r.channel, r.avg_messages_per_user]))
  return (
    <ChartShell
      title={t("Engagement Rates")}
      infoText={t("Average chat message count per user.")}
      loading={loading}
      hasData={rows.length > 0}
      chartMinWidth={rows.length * 42}
      onDownloadCSV={handleCSV}
      png={{
        title: `Engagement Rates (messages/user) - ${group} - ${month} - holochatstats.info`,
        filename: "engagement_rates.png",
      }}
      controls={
        <>
    ...
    >
      <ChartContainer config={config} className="h-full w-full !aspect-auto">
        <BarChart data={rows} margin={{ top: 30, right: 20, left: 0, bottom: 80 }}>
          ...
          <Bar dataKey="avg_messages_per_user" fill="var(--color-avg_messages_per_user)" radius={[2, 2, 0, 0]} />
        </BarChart>
      </ChartContainer>
    </ChartShell>
  )
